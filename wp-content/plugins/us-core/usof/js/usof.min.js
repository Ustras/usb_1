...
this.colors.state=state;this.$switchers.filter('[value="'+this.colors.state+'"]').prop('checked','checked');this.change(gradient,setColor)},hide:function(){var that=this;that.$colpick.css('display','none').removeClass('type_gradient');clearTimeout(that.timeout);if(that.colors.initialColor!=that.$input.val()){that.$input.trigger('change')}
that.$hueContainer.off('mousedown touchstart');that.$gradientHueContainer.off('mousedown touchstart');that.$curentColorBox.off('mousedown touchstart');that.$gradientColorBox.off('mousedown touchstart');that.$alphaContainer.off('mousedown touchstart');that.$gradientAlphaContainer.off('mousedown touchstart');that.$angleContainer.off('mousedown touchstart');that.$palette.off('mousedown');that.$switchers.off('change');that.$input.off('blur').trigger('blur');that.$colpick.remove()},isValidColor:function(value){var gradient,valueG2,valueG1;if(typeof value=='string'){if(this.colorNameToHex(value)){value=this.hexToHsba(this.colorNameToHex(value))}else if(this.isGradient(value)){gradient=this.gradientParser(value);value=this.hexToHsba(gradient.hex);this.colors.gradient=gradient}else if(value=='transparent'){value={h:360,s:0,b:0,a:0,}}else{value=this.hexToHsba(this.normalizeHex(value))}}else if(value.r!=undefined&&value.g!=undefined&&value.b!=undefined){value=this.rgbaToHsba(value)}else{return!1}
...
return value}}}(jQuery);!function($,undefined){var _window=window,_document=document;if(_window.$usof===undefined){return}
$usof.field.custom_dropdown={init:function(){this.$container=$('.usof-custom-dropdown',this.$row);this.$options=$('.usof-custom-dropdown-item',this.$container);this.$options.on('click',this._events.selectItem.bind(this))},_events:{selectItem:function(e){var $selected=$(e.currentTarget);this.setValue($selected.data('value')||null,!0)}},setValue:function(value){this.$options.removeClass('current').filter('[data-value="'+value+'"]').addClass('current',!0);this.$input.val(value);this.trigger('change',value)},getValue:function(){return this.$input.val()||''}}}(jQuery);!function($,undefined){var _window=window,_document=document;if(_window.$usof===undefined){return}
...
this.frame.open()},_baseName:function(path){return(''+path).substring(path.lastIndexOf('/')+1)}}}(jQuery);!function($,undefined){var _window=window,_document=document;if(_window.$usof===undefined){return}
...
if(this.extStates.length){$('[data-responsive-state-content="default"]',this.$container).each(function(_,content){var $content=$(content);this.extStates.map(function(responsiveState){var $cloneContent=$content.clone().attr('data-responsive-state-content',responsiveState).addClass('hidden');$content.after($cloneContent)}.bind(this))}.bind(this))}
...
field.trigger('beforeShow').on('change',this._events.changeValue.bind(this))}.bind(this))}.bind(this));var pid=setTimeout(function(){if(!this.inited){this.setValue(this.$input.val());this.checkChangeValues.call(this)}
this.$importBtnCopy.prop('disabled',!this.getValue());clearTimeout(pid)}.bind(this),1);this.$container.find('.us-builder-states').toggleClass('hidden',!this.extStates.length);this.$container.on('click','[data-accordion-id]',this._events.toggleAccordion.bind(this)).on('click','.usof-design-options-reset',this._events.resetValues.bind(this)).on('click','.usof-design-options-responsive',this._events.toggleResponsive.bind(this)).on('click','[data-responsive-state]',this._events.changeResponsiveStates.bind(this));this.$document.on('usb.setResponsiveState',this._events.usbSetResponsiveState.bind(this));this.$import.on('click','button[data-action]',this._events.importActions.bind(this))},_events:{changeValue:function(field){var resultValue={};$.each(this.groupParams,function(responsiveState,groupParams){var groupValues=groupParams.getValues();$.each(groupValues,function(param,value){var defaultValue=this.defaultValues[responsiveState][param];if(value!==defaultValue){if(!resultValue.hasOwnProperty(responsiveState)){resultValue[responsiveState]={}}
...
this._debounce=setTimeout(function(){this.checkChangeValues.call(this);clearTimeout(this._debounce)}.bind(this),1);this.$importBtnCopy.prop('disabled',!resultValue);this.trigger('changeDesignField',field)},resetValues:function(e){e.stopPropagation();var $target=$(e.currentTarget),$groupHeader=$target.closest('[data-accordion-id]'),groupName=$groupHeader.data('accordion-id');if($groupHeader.hasClass('responsive')){this._events.toggleResponsive.call(this,e)}
if(this.defaultGroupValues.hasOwnProperty(groupName)){$.each(this.defaultGroupValues[groupName],function(responsiveState,defaultValues){var groupParams=this.groupParams[responsiveState];groupParams.setValues($.extend(groupParams.getValues(),defaultValues));$.each(defaultValues,function(groupParams,name){var fields=groupParams.fields;if(fields.hasOwnProperty(name)&&fields[name].hasOwnProperty('_data')&&fields[name]._data.hasOwnProperty('relations')){var $link=$('i.fas',groupParams.$fields[name]);if($link.length&&$link.is('.fa-link')){$link.trigger('click')}}}.bind(this,groupParams))}.bind(this))}
var pid=setTimeout(function(){$groupHeader.removeClass('changed');clearTimeout(pid)},1000*0.5)},watchAttrLink:function(field,e,state){var $target=$(e.currentTarget),isUnlink=$target.is('.fa-unlink'),relations=[];if(state!==undefined){isUnlink=state}
...
if(this.groupParams.hasOwnProperty(responsiveState)&&this.groupParams[responsiveState]instanceof $usof.GroupParams){this.groupParams[responsiveState].setValues(values)}
